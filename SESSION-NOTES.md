...
## 2026-05-30 — Boot crash SOLVED. App runs on Anthony's iPhone.

### TL;DR

Three days of debugging are over. The app boots, the onboarding screen renders on Anthony's iPhone via the local dev-client build. There were *two* separate crashes hiding under one symptom:

1. **Native crash** (fixed 2026-05-28, commit `09bb1e7`): missing `react-native-worklets` peer dep that Reanimated 4 requires. Found via `expo-doctor`.
2. **JS render crash** (fixed today, commit `d7352fe`): duplicate copies of `@react-navigation/*` packages — once as our direct deps, once as expo-router 6's bundled deps — fought for navigator-context ownership, making `SceneView` resolve to `undefined`. The fix: **remove all `@react-navigation/*` entries from `mobile/package.json`**, delete `bun.lock`, run `bun install`. Only `@react-navigation/native` is actually imported in our `src/`, and expo-router bundles a newer copy.

### The research moment that broke the impasse

After 6+ hours of bisecting (stub all screens → still crashes, stub layouts → still crashes, swap NativeWind versions → still crashes, ran `expo install --fix` → still crashes), Anthony said: *"Stop proposing fixes. Search for real reports of this exact error with our stack."*

The first GitHub hit ([expo/expo #43744](https://github.com/expo/expo/issues/43744)) was almost exactly our situation — SDK 55 instead of 54, but identical symptom: bare `<Stack/>` layout still throws "Element type invalid: undefined ... SceneView". Two independent commenters (Stephane-Trustie + papalardo) confirmed the same fix: remove duplicate `@react-navigation/*` from package.json.

Lesson, recorded for next time: **when bisection / version-bumping fails to converge, switch to web research before doing more attempts.** Real bug reports on the same stack outperform any amount of clever reasoning.

### Live state right now

- Dev client is installed on Anthony's iPhone (built via Xcode `Plannr.xcworkspace`, not EAS).
- Metro is running at `http://192.168.1.17:8081`. The phone's dev client connects there.
- `mobile/.env` was pointing at an old local-backend LAN IP (`http://192.168.1.38:3000`), so sign-in throws "network request failed". **Updated `mobile/.env` to use the Railway URL** so the dev build talks to the live backend. `.env` is gitignored so this is Anthony-machine-local. Restarted Metro `--clear` for the env var to bake into the bundle.
- Anthony hasn't yet confirmed end-to-end sign-up against Railway from the phone. **That's next session's first test.**

### How to resume

1. Backend healthy: `curl https://plannr-production-9a90.up.railway.app/health` should be `{"status":"ok"}`.
2. From `mobile/`: `bunx expo start --dev-client --clear`. Wait for "Waiting on http://localhost:8081".
3. Force-quit Plannr on iPhone and reopen. It'll auto-connect to the saved URL `http://192.168.1.17:8081` (if Mac's LAN IP changed, update accordingly).
4. Onboarding/sign-in screen should appear. Test full sign-up → it should hit Railway and succeed.
5. If sign-up works, move to **Gate 4 finish**: a fresh `eas build --profile preview --platform ios` for installable distribution (and eventually TestFlight). The dev-client install was the diagnosis tool; the EAS preview build is what you'd actually distribute.

### Remaining cleanup (low priority)

- `package.json` `scripts` block has `"android": "expo run:android"` and `"ios": "expo run:ios"` (prebuild side effect). Harmless but reflects that `ios/` directory now exists (gitignored).
- `ios/` native project was generated by `expo prebuild` for the local dev build. It's gitignored, so EAS cloud builds continue to regenerate it on their servers — unaffected.
- expo-router internal diagnostic probes (RT2/RT3/RT4) and `_layout.tsx` console.logs (ROOT-DIAG/LAYOUT-DIAG) — all already removed.
